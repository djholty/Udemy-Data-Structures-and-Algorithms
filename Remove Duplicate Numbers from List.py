...
#     Create an empty list unique_lst to store the unique elements, and an empty 
# set seen to keep track of the elements already processed.

#     Iterate through the input list lst using a for loop.

#     For each item in lst, check if it is not in seen. If it's not in seen, 
# append it to unique_lst and add it to the seen set.
...
#         If the item is not in the set, appending it to the list unique_lst: O(1) 
# on average
...
# inside the loop have an average time complexity of O(1). Therefore, the overall 
# time complexity is O(n).

# Space complexity:

# O(n), as we're using additional data structures (list and set) to store unique 
# elements.