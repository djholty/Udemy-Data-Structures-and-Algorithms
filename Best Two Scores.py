...
#         Define a function called first_second that takes a list called my_list 
#         as an argument.

#     max1, max2 = float('-inf'), float('-inf')

#         Initialize two variables max1 and max2 to store the first and second best 
#         scores. Set their initial values to negative infinity.
...
#         If the current element is greater than max1, update max2 to the current 
#         value of max1.
...
#         Set max2 to the current element, as it is now the second-highest 
#         value found so far.
...
# The function iterates through the list my_list once, with each iteration taking 
# constant time O(1) to perform comparisons and updates. Since there are n elements 
# in the list, the overall time complexity of the function is O(n).
...
# depend on the size of the input list. Therefore, the space complexity is O(1), 
